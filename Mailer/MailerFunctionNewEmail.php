...
function newEmail($nombre,$destinatario, $asunto, $mensaje)
{
...
<head>
    ...
    try {
        ...
        if (!$mail->send()) {
            $response = array('status' => 'error', 'message' => 'Error al enviar el correo electrónico.'. $mail->ErrorInfo);
        } else {
            $response = array('status' => 'success', 'message' => 'Correo enviado exitosamente.');
        }

    } catch (Exception $e) {
        $response =  array('status' => 'error', 'message' => 'Exception'. $e . 'Error:' . $mail->ErrorInfo);
    }
    return $response;
}
